...
module.exports = async function(args={}){
	...
	smarts.gosmart(args, 'host', '127.0.0.1')
	...
	let uri = Uri(uriArgs)
	let connection = await MongoClient.connect(`mongodb://test1:test1@127.0.0.1:27017/admin?authSource=admin`, { useNewUrlParser: true }).catch(err=>console.error(err))
	if(connection){
		...
		uri = Uri(uriArgs)
		connection = await MongoClient.connect(uri).catch(err=>console.error(err))
		if(connection){
			console.log(`connected to ${uri} with auth ${uriArgs['auth']} successfully`)
			delete uriArgs['password']
			console.log('uriArgs', uriArgs)
